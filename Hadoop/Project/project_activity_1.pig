-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/bhavya/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/bhavya/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/bhavya/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/bhavya/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/bhavya/outputs' USING PigStorage('\t');